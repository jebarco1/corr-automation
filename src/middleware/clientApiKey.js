...
export function requireClientApiKey(req, res, next) {
  if (!appConfig.clientApiKeys.length) {
    return res.status(503).json({
      error: "Client API key authentication is not configured.",
      configuration: "Set CORR_CLIENT_API_KEYS in the server .env file.",
      generate: "npm run key:generate"
    });
  }
